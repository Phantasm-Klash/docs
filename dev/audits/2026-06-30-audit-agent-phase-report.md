...
- docs 方向在补管理闭环：project-manager-agent 的 #30 已合并进 `origin/main`，manager 现在能输出 stale PR group 与 merge-ready 摘要；#28 继续承载 audit-agent 资源风险和队列审计报告。
...
- `docs`：当前分支 `agent/audit-agent/resource-risk-20260630-1022` 已合并最新 `origin/main`，PR #28 `Summarize agent resource risk` 仍打开。project-manager-agent PR #30 `Surface stale PR supersede groups` 已合并为 `ab8bed9`。
...
- 当前 open PR 数：10。
- docs #28：本轮已合并最新 `origin/main` 并继续使用该 PR 承载审计报告更新；推送后需要等待 `docs-audit` / `auto-merge` 重新跑完。
- docs #30：已合并，新增 stale PR group 与 merge-ready PR 摘要能力，方向正确。
...
- `project-manager-agent`：运行中；最新 final 完成 `supersede_groups`、merge-ready PR 摘要与 brief mail wording 修复，docs #30 已合并。
...
- #22 服务端 PR 有 `server-contract-tests` 与 `auto-merge` 通过；#18 战斗服 PR 有 `battle-server-checks` 与 `auto-merge` 通过；docs #30 合并前 `docs-audit` 与 `auto-merge` 通过，docs #28 推送更新后需等待新一轮 checks。
...
- project-manager-agent：#30 已合并；继续让 manager summary 正式输出 stale group 与 merge-ready 项，并把 SpellKard #13-#19 转成可执行清退表。
